  ...
  return (
    <>
      <Row className="media-container-row">
        <h4 className="card-title align-left py-2 mbr-bold mbr-fonts-style mbr-text align-center display-7">Diagnosis</h4>
        <Col md={12} className="p-3 align-left">
          <Row className="media-container-row">
            <Col md={1} className="align-left"></Col>
            <Col md={6} className="align-left">
              <Formik initialValues={initialDiagnosisValues} validationSchema={validationSchema} onSubmit={handleSubmit} enableReinitialize={true}>
                {({ setFieldValue }) => {
                  useEffect(() => {
                    const fetchDiagnosisDetailData = async () => {
                      const [data, err] = await getDiagnosisDetailsAPI(diagnosisId);
                      if (data) {
